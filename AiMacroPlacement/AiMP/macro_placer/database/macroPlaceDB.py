...
class MacroPlaceDB(object):
    ...
    def setup_rawdb(self, params):
        ...
        if self.pydb is None:    
            ...
            self.pydb = ieda_dm.pydb(self.get_dmInst_ptr)
    ...
    def virtual_net_init(self):
        ...
        for macro_connection in macro_connections:
            print(
                "src macro name {} -> snk macro name {} stages {} hop {}".format(
                    ...
                    " ".join([str(x) for x in macro_connection.stages_each_hop]),
        ...
        for macro_pair, weight in macro_pair_weights.items():
            ...
            self.pin_offset_x = np.append(self.pin_offset_x, self.node_size_x[src_inst_id] / 2)
            self.pin_offset_y = np.append(self.pin_offset_y, self.node_size_y[src_inst_id] / 2)
            self.node2pin_map[src_inst_id] = np.array(np.append(self.node2pin_map[src_inst_id], src_pin_id), dtype=np.int32)
            self.pin2node_map = np.append(self.pin2node_map, src_inst_id)
            self.pin2net_map = np.append(self.pin2net_map, net_id)
            
            dst_pin_id = len(self.pin_offset_x)
            self.pin_offset_x = np.append(self.pin_offset_x, self.node_size_x[dst_inst_id] / 2)
            self.pin_offset_y = np.append(self.pin_offset_y, self.node_size_y[dst_inst_id] / 2)
            self.node2pin_map[dst_inst_id] = np.array(np.append(self.node2pin_map[dst_inst_id], dst_pin_id), dtype=np.int32)
            ...
            self.flat_net2pin_start_map = np.append(self.flat_net2pin_start_map, int(len(self.flat_net2pin_map)))
            self.flat_net2pin_map = np.append(self.flat_net2pin_map, src_pin_id)
            self.flat_net2pin_map = np.append(self.flat_net2pin_map, dst_pin_id)
            self.net2pin_map.append(np.array([src_pin_id, dst_pin_id], dtype=np.int32))
            self.net_weights = np.append(self.net_weights, weight)
        ...
        self.flat_net2pin_start_map = np.array(self.flat_net2pin_start_map,dtype=np.int32)
...
""" % (
        ...
        total_movable_cell_area = self.total_movable_node_area - self.total_movable_macro_area
        ...
        content += "total_movable_node_area = %g, total_fixed_node_area = %g, total_space_area = %g\ntotal_movable_cell_area = %g, total_movable_macro_area = %g\n" % \
                (self.total_movable_node_area, self.total_fixed_node_area, self.total_space_area, total_movable_cell_area, self.total_movable_macro_area)
    ...
    def update_macros(self, params, area_threshold=10, height_threshold=2):
        ...
        self.is_pin_lower_x = (np.array(macro_pin_offset_x_mean) <= 0.1* self.node_size_x[self.movable_slice][self.movable_macro_mask]).astype('float64')
        self.is_pin_upper_x = (np.array(macro_pin_offset_x_mean) >= 0.9* self.node_size_x[self.movable_slice][self.movable_macro_mask]).astype('float64')
        self.is_pin_lower_y = (np.array(macro_pin_offset_y_mean) <= 0.1* self.node_size_y[self.movable_slice][self.movable_macro_mask]).astype('float64')
        self.is_pin_upper_y = (np.array(macro_pin_offset_y_mean) >= 0.9* self.node_size_y[self.movable_slice][self.movable_macro_mask]).astype('float64')
        ...
        params.macro_pin_halo_x = self.crop_to_site(params.macro_pin_halo_x, "x")
        params.macro_pin_halo_y = self.crop_to_site(params.macro_pin_halo_y, "y")
        ...
        if params.macro_pin_halo_x >= 0:
            self.node_size_x[self.movable_macro_idx] += self.is_pin_lower_x * params.macro_pin_halo_x + self.is_pin_upper_x * params.macro_pin_halo_x
            self.node_size_y[self.movable_macro_idx] += self.is_pin_lower_y * params.macro_pin_halo_y + self.is_pin_upper_y * params.macro_pin_halo_y
            
            self.node_x[self.movable_macro_idx] -= self.is_pin_lower_x * params.macro_pin_halo_x
            self.node_y[self.movable_macro_idx] -= self.is_pin_lower_y * params.macro_pin_halo_y
            
            self.pin_offset_x[self.movable_macro_pins] += self.is_pin_lower_x[self.pin2node_map[self.movable_macro_pins]] * params.macro_pin_halo_x
            self.pin_offset_y[self.movable_macro_pins] += self.is_pin_lower_y[self.pin2node_map[self.movable_macro_pins]] * params.macro_pin_halo_y
    ...
    def apply(self, params, node_x, node_y):
        ...
        # unscale locations 
        unscale_factor = 1.0 / params.scale_factor
        node_x = self.node_x[:self.num_movable_nodes] * unscale_factor + params.shift_factor[0]
        node_y = self.node_y[:self.num_movable_nodes] * unscale_factor + params.shift_factor[1]
        # update raw database 
