import os
import sys
sys.path.append(os.environ['ROOT-PATH'])
sys.path.append(os.environ['ROOT-PATH'] + '/app')
sys.path.append(os.environ['ROOT-PATH'] + '/app/AiMP')
...
from aimp.aifp.database.data_structure.observation_space import ObservationSpace
from aimp.aifp.operation.macro_placer.rl_placer.strategy.macro_sort import MacroSort
from aimp.aifp.operation.macro_placer.rl_placer.reward.placement_reward import PlacementReward
from aimp.aifp.operation.macro_placer.rl_placer.reward.heuristic_reward import HeuristicReward
from aimp.aifp.database.data_structure.fp_solution import FPSolution, MacroInfo
...
from aimp.aifp.utility.operators import number_digits
from aimp.aifp.utility.draw import draw_layout
import json
# from thirdparty.irefactor import py_aifp_cpp as aifp_cpp
from aimp.aifp import setting
from aimp.aifp.operation.data_io import aifp_db_io
...
class LocalEnv:
    def __init__(self):
        print("local env init ok")
        pass

    def init(self,  mp_db, is_evaluate=False):
        case_select = setting.case_select
        self._is_evaluate=is_evaluate
        # mp_db = pickle.loads(dumped_mp_db)
        print('here ok')
        self._database = self._init_env_database(mp_db)
        ...
        self._init_fp_solution()
        print("===== base env:   macro num {}, unfixed_macro_num: {}, node num {}".format(self.macro_nums, self.unfixed_macro_nums, self.node_nums))

    def _init_fp_solution(self):
        self._fp_solution = FPSolution()
        self._fp_solution.set_design_name(setting.case_select)
        for macro in self._database.get_macro_list():
            self._fp_solution.add_macro_info(
                MacroInfo(
                    macro.get_name(),
                    macro.get_low_x(),
                    macro.get_low_y(),
    ...
    def _update_fp_solution(self):
        for macro in self._database.get_macro_list():
            self._fp_solution.update_macro_info(
                macro_name = macro.get_name(),
                low_x = macro.get_low_x(),
                low_y = macro.get_low_y(),
                orient = macro.get_orient()
            )
        self._fp_solution.set_description('rl-iteration-{}'.format(self._iteration))
        self._fp_solution.set_score_dict(self._score_dict)

    # def set_mp_db(self, mp_db):
    #     self._mp_db = pickle.loads(mp_db)
    # def set_xl(self, xl:int):
    #     self._xl = xl
    # def set_yl(self, yl):
    #     self._yl = yl
    ...
    # def set_num_movable(self, num_movable):
    #     self._num_movable = num_movable
    # def set_node_x(self, node_x):
    #     self._node_x = node_x
    # def set_node_y(self, node_y):
    #     self._node_y = node_y
    # def set_node_size_x(self, node_size_x):
    #     self._node_size_x = node_size_x
    # def set_node_size_y(self, node_size_y):
    #     self._node_size_y = node_size_y
    # def set_net2pin_map(self, net2pin_map):
    #     self._net2pin_map = net2pin_map
    # def set_pin2node_map(self, pin2node_map):
    #     self._pin2node_map = pin2node_map
    # def set_pin_offset_x(self, pin_offset_x):
    #     self._pin_offset_x = pin_offset_x
    # def set_pin_offset_y(self, pin_offset_y):
    #     self._pin_offset_y = pin_offset_y
    ...
    def _get_episode_info(self):
        ...
        if self._step == self.episode_length:
            info['fp_solution'] = self._fp_solution
        return info
    ...
    def _get_unfixed_macro_indices(self):
        ...
        return [idx for idx in self._database.get_macro_indices() if inst_list[idx].get_status() == "unfixed"]
    ...
    #     print('========== load from aimp.aifp-db success ==========')
    #     return RLEnvDB(design_data_dict)

    def _init_env_database(self, mp_db):
        # design_data_dict = aifp_db_io.read_from_imp_db(mp_db)
        # print('start init env db')
        # print('xl: ', self._xl)
        # print('yl: ', self._yl)
        # print('width: ', self._width)
        # print('height: ', self._height)
        # print('num_nodes: ', self._num_nodes)
        # print('num_movable: ', self._num_movable)
        # print('node_x: ', self._node_x)
        # print('node_y: ', self._node_y)
        # print('node_size_x: ', self._node_size_x)
        # print('node_size_y: ', self._node_size_y)
        # print('pin2node_map: ', self._pin2node_map)
        # print('net2pin_map: ', self._net2pin_map)
        # print('pin_offset_x: ', self._pin_offset_x)
        # print('pin_offset_y: ', self._pin_offset_y)
        # design_data_dict = aifp_db_io.read_from_numpy_data(
        #     self._xl,
        #     self._yl,
        #     self._width,
        #     self._height,
...
#         self._env_logger.root.name = 'aifp:env_{}'.format(self._env_name)
#         self._env_logger.info('========= env server {} log ========='.format(self._env_name))
#         self._env_logger.info('feature edge num: {}'.format(self._database.num_edges))
#         self._env_logger.info('macro num: {}'.format(self._database.num_macros))
#         self._env_logger.info('blockage num: {}'.format(self._database.num_blockages))
#         self._env_logger.info('episode_length: {}'.format(self.episode_length))
#         self._start_time = time.time()
...
#         return self._database.num_node_feature_dim
...
#         action_mask = action_mask.astype(self.observation_space.action_mask.dtype)
#         node_features = np_obs_data_dict['nodedata'].astype(self.observation_space.node_features.dtype)
#         layout_features = np_obs_data_dict['layoutdata'].astype(self.observation_space.layout_features.dtype)

#         return [node_features, layout_features, macro_idx_to_place, sparse_adj_i, sparse_adj_j, sparse_adj_weight, action_mask]
...
#             done = 0.0
#             reward, self._score_dict = self._get_reward(macro_idx=placing_macro_idx, pr_evaluate=False)
#             self._return += reward
...
#             action_mask = np.zeros(self.observation_space.action_mask.shape, dtype=self.observation_space.action_mask.dtype)
#             done = 1.0
#             reward, self._score_dict = self._get_reward(macro_idx=placing_macro_idx, pr_evaluate=True)
#             self._return += reward
#             self._score_dict['return'] = self._return
#             self._update_fp_solution()
...
#                 if (self._iteration+1) % setting.rl_config['save_figure_interval'] == 0:
#                     self._env_logger.info('drawing layout result...\n')
#                     self._draw_result(save_path='{}/iter_{}.png'.format(self._figure_dir, self._iteration))

            
#             # if setting.rl_config['save_to_dataset']:
#             #     self._save_to_dataset(reward)

#         np_obs_data_dict = self._database.get_concat_obs_data(macro_idx_to_place)
#         info = self._get_episode_info()
...
#         return node_features, layout_features, macro_idx_to_place, sparse_adj_i, sparse_adj_j, sparse_adj_weight, action_mask, reward, done, info
#         # return [node_features, macro_idx_to_place, sparse_adj_i, sparse_adj_j, sparse_adj_weight, action_mask], reward, done, info

#     def _draw_result(self, save_path, draw_edge=False):
#         start_time = time.time()
#         draw_layout(save_path, self._database, draw_edge)
#         print('draw_time:', time.time() - start_time)
...
#     def _get_reward(self, macro_idx, pr_evaluate):
#         cost, score_dict = self._evaluator.evaluate(
#             env_db=self._database, 
#             pr_evaluate=pr_evaluate,
#             macro_idx=macro_idx)
#         return -cost, score_dict
...
#     def _init_observation_space(self):
#         return ObservationSpace(self.node_nums, self.edge_nums)
#         # self.get_database().num_node_feature_dim
