...
# from aimp.aifp.operation.macro_placer.rl_placer.environment.env_cluster_controller import EnvClusterController
...
# import aimp.aifp.operation.macro_placer.rl_placer.environment.remote.remote_env_pb2 as remote_env_pb2
# import aimp.aifp.operation.macro_placer.rl_placer.environment.remote.remote_env_pb2_grpc as remote_env_pb2_grpc
...
@singleton
class ParallelEnv:
    def __init__(self, mp_db):

        # log_to_driver=False # Task and actor logs will not be copied to the driver stdout.
        # dumped_mp_db = pickle.dumps(mp_db)
        print('dump ok')
        self._env_nums = setting.get_env_nums()
        self._init_ray_config()
        print('env_num: ', self._env_nums)
        RemoteEnv = ray.remote(LocalEnv)
        #.options(memory=2e9) # num_cpus=1, num_gpus=0
        self._remote_envs = [RemoteEnv.remote() for i in range(self._env_nums)]


        for i in range(len(self._remote_envs)):
            # self._remote_envs[i].set_xl.remote(mp_db.xl)
            # self._remote_envs[i].set_yl.remote(mp_db.yl)
            # self._remote_envs[i].set_width.remote(mp_db.width)
            # self._remote_envs[i].set_height.remote(mp_db.height)
            # self._remote_envs[i].set_num_nodes.remote(mp_db.node_x.shape[0])
            # self._remote_envs[i].set_num_movable.remote(mp_db.num_movable_nodes)
            # self._remote_envs[i].set_node_x.remote(mp_db.node_x)
            # self._remote_envs[i].set_node_y.remote(mp_db.node_y)
            # self._remote_envs[i].set_node_size_x.remote(mp_db.node_size_x)
            # self._remote_envs[i].set_node_size_y.remote(mp_db.node_size_y)
            # self._remote_envs[i].set_net2pin_map.remote(mp_db.net2pin_map)
            # self._remote_envs[i].set_pin2node_map.remote(mp_db.pin2node_map)
            # self._remote_envs[i].set_pin_offset_x.remote(mp_db.pin_offset_x)
            # self._remote_envs[i].set_pin_offset_y.remote(mp_db.pin_offset_y)
            self._remote_envs[i].init.remote(mp_db)

        print("remote_envs ok!")
    ...
    def _init_ray_config(self):
        runtime_env={"working_dir": "/home/liuyuezuo/ai-eda/app",
            "env_vars": {"AIFP_PATH": "/home/liuyuezuo/RLMP_PROJECT"}}
